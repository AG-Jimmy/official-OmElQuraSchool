"use strict";
const bestStudentInterface = require("../database/bestStudentInterface");

const BestStudentDB = require("../database/databaseDB");
const BestStudentDatabase = new BestStudentDB();

// console.log(BestStudentDB.counter);
module.exports = {
  get: async (req, res) => {
    try {


      const getBestStudent = await BestStudentDatabase.getBestStudent();
      if (getBestStudent) res.status(200).json(getBestStudent);
    } catch (error) {
  ...
  post: async (req, res) => {
    try {
      ...
      const bestStudent = await BestStudentDatabase.postBestStudent({
        name,
        classroom,
        rating,
      });
  ...
  delete: async (req, res) => {
    try {
      const id = req.params.id;
      const deleteBestStudent = await bestStudentInterface.deleteBestStudent(
        id
      );
...
module.exports = {
  ...
  //   try {
  //     const { name, classroom, rating } = req.body;
  //     const bestStudent = await bestStudentInterface.setBestStudent({
  //       name,
  //       classroom,
  //       rating,
  //     }); //.toObject();
